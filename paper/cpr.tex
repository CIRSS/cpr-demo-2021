...
Once a recorded run is complete, the \programname{cpr} command-line utility extracts these OS-level records from the \programname{ReproZip} trace, transform them into RDF triples, and loads the triples into an RDF dataset in an instance of Blazegraph\footnote{https://github.com/blazegraph/database}. The triples are expressed using a vocabulary developed to represent provenance information in the context of Whole Tale recorded runs (\Figref{fig:cpr-vocab}). 
